<template>
  <div class='register'>
    ...
    <div class="content">
      <div class="left">
        <img :src="`data:image/jepg;base64,` + hospitalStore.hospitalInfo.hospital?.logoData" alt="Logo">
      </div>
      <div class="right">
        <div>挂号规则</div>
        <div class="time">
          <span>预约周期：10天 放号时间:{{ hospitalStore.hospitalInfo.bookingRule?.releaseTime }} 停挂时间:{{
            hospitalStore.hospitalInfo.bookingRule?.stopTime }}</span>
        </div>
        <div class="address">
    ...
    <div class="department">
      ...
      <div class="departmentInfo">
        ...
        <div class="showDepartment" v-for="department in hospitalStore.departmentArr" :key="department.depcode">
          ...
          <!-- 小科室内容 -->
          <ul @click="login(item)" v-for="item in department.children" :key="item.depcode">
            <li>{{ item.depname }}</li>
          </ul>
        </div>
      </div>
...
const changeIndex = (index: number) => {
  ...
  let allH1: any = document.querySelectorAll('.cur')
      ...
      ul {
        ...
        li {
          ...
          &.active {
            ...
            font-weight: 800;
            font-size: 18px;
            background-color: rgba(234, 226, 226, 0.445);
          }
        }
